<?php
//FOR WHEN STREAM REQUIRES OAUTH (NOTE: ADD authString() FUNCTION, OTHER CHANGES to Pirehose--THIS
//GENERAL VERSION IS OUT OF DATE
...
class Stream extends Phirehose{
	//mySQL constants
	protected $my_sql_server = ':/Applications/MAMP/tmp/mysql/mysql.sock';
	protected $my_sql_user = 'root';
	protected $my_sql_pwd = 'root';	
	protected $db = 'twitter';
	protected $table = 'test';
	protected $link;
	protected $searchID;
	
	public function __construct($credentials, $method = Phirehose::METHOD_FILTER, $format = self::FORMAT_JSON){
    	...
    	$this->method = $method;
    	$this->format = $format;
    	$this->link = mysql_connect($this->my_sql_server, $this->my_sql_user, $this->my_sql_pwd) or die(mysql_error());
		...
		$this->searchID = 1+mysql_result($result, 0);
	...
	public function enqueueStatus($status){
		...
		//Set values
		$values = "'".$this->getTweetID($data);
		$values.= "','".$this->getTweetDate($data);
		$values.= "','".$this->getTweetSource($data);
		$values.= "','".$this->getTweetText($data);
		$values.= "','".$this->getTweetLat($data);
		$values.= "','".$this->getTweetLong($data);
		$values.= "','".$this->getUserID($data);
		$values.= "','".$this->getScreenName($data);
		$values.= "','".$this->getUserLocation($data);
		$values.= "','".$this->getSearchID();
		$values.= "'";
		
		$sqlInsert = "insert into $this->table ($columns) values ($values)";
		mysql_query($sqlInsert, $this->link);
	}
	
	//Inserts search info into the mySQL database
	public function insertSQLsearch(){
		$table = 'searches';
	...
	//Get tweet info
	private function getTweetID($data){
		return $data['id'];
	}
	
	private function getTweetDate($data){
	...
	private function getTweetSource($data){
		$start = strpos($data['source'],'>');
		if($start === false)
			return $data['source'];
		else{
			$start++;
			$end = strpos($data['source'],'<',$start);
			$source = substr($data['source'],$start,$end-$start);
			return $source;
		}
	}
	
	private function getTweetText($data){
		return $data['text'];
	}
	
	private function getTweetLat($data){
		if($data['coordinates'] != '')
	...
	private function getTweetLong($data){
		if($data['coordinates'] != '')
	...
	private function getScreenName($data){
	...
	private function getUserLocation($data){
		return $data['user']['location'];
	}
	
	
	//Get search info
	private function getSearchID(){
...
$credentials = array('consumer_key' => '8rGqiXg7uP2fhBpsjy75w',
...
$stream = new Stream($credentials);
$stream->setTrack(array($track));
$stream->insertSQLsearch();
$stream->consume();
...
abstract class Phirehose
{
   ...
   * @param string $format
   */
  public function __construct($credentials, $method = Phirehose::METHOD_SAMPLE, $format = self::FORMAT_JSON)
  {
    $this->credentials = $credentials;
    $this->method = $method;
    $this->format = $format;
  ...
  protected function connect() 
  {
    ...
    do {
      ...
      fwrite($this->conn, $this->authString($url, $this->credentials)."\r\n");
...
//Returns an OAuth header string given a URL
function authString($baseURL, $credentials, $httpMethod = 'POST'){
	$sig_key = urlencode($credentials['consumer_secret']).'&'.urlencode($credentials['oauth_token_secret']);
	$signArray = array('oauth_consumer_key' => $credentials['consumer_key'],
	...
	$query = http_build_query($signArray);
	...
	$authString = 'Authorization: OAuth realm="'.$baseURL.'",
