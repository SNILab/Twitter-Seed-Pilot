...
class Stream extends Phirehose{
	//mySQL constants
	protected $my_sql_server = 'research.bowdoin.edu';
	protected $my_sql_user = 'ruivoss';
	protected $my_sql_pwd = 'rtwitps1';	
	protected $db = 'twitter';
	protected $table = 'tweets';
	protected $link;
	protected $searchID;
	
	public function __construct($username, $password, $runTime, $method = Phirehose::METHOD_FILTER, $format = self::FORMAT_JSON){
    	//Twitter time (roughly)
		date_default_timezone_set('UTC');
		
		$this->username = $username;
    	$this->password = $password;
    	$this->endTime = time() + $runTime;
    	$this->method = $method;
    	$this->format = $format;
	...
	public function enqueueStatus($status){
		$data = $this->json_decode($status, true);
	...
	private function getSearchTerms(){;
		return implode(',', $this->trackWords);
	}
	...
	private function json_decode($content, $assoc=false){
		require_once 'php-modules/JSON.php';
        if($assoc) {
...
//MAIN:
//7: Twitter constants
$username = 'PolarSociology7';
$password = 'bowdoincollege';

////


$track = 'cancer survivor';
$time = 2592000;
...
$stream = new Stream($username, $password, $time);
$stream->setTrack(explode(',', $track));
...
abstract class Phirehose
{
   ...
   * @param string $runTime Any runtime in seconds
   * @param string $method
   * @param string $format
   */
  public function __construct($username, $password, $runTime, $method = Phirehose::METHOD_FILTER, $format = self::FORMAT_JSON)
  ...
  public function consume($reconnect = TRUE)
  {
    ...
    do {
      ...
      while ($this->conn !== NULL && !feof($this->conn) && ($numChanged = stream_select($this->fdrPool, $fdw, $fde, $this->readTimeout)) !== FALSE) {        
        ...
        //Check time to see if ready to disconnect(SL)
      ...
      // Some sort of socket error has occured if time is not up(SL)
      if($timeUp !== true){
    ...
    // Exit
    if($timeUp === true)
    	$this->log("Time is up:   EndTime: $this->endTime   ActualTime: ".time());
    $this->log('Exiting.');
